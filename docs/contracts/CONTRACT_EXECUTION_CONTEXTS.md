...
9. A `resource` block scopes part of a body.  A `resource memory` block
   that is a function's whole body is a domain policy hidden from every
   caller -- each pays a copy of the result, none can share a domain
   across calls -- and is diagnosed (a warning: the program is sound);
   the block belongs at the call site.
