...
| §7 prelude | conforms (Task 31) | partial (Tasks 38, 41): scope order, shadowing, identity, reserved names, prelude method and `extend` visibility, and derived conformance through the prelude conform; prelude files load partially; the `hir`/`mir`/`llvm` probe columns are not yet measured with the prelude |
